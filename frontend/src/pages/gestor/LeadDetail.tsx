...
export function LeadDetail() {
    ...
    const handleConverter = async () => {
        ...
        try {
            ...
            alert('Lead convertido em beneficiario com sucesso!');
        } catch (err: any) {
            alert(err.response?.data?.detail || 'Erro ao converter lead');
        } finally {
            setActionLoading(false);
        }
    };
                        ...
                        <div className="space-y-2">
                            {lead.status !== 'CONVERTIDO' && lead.status !== 'PERDIDO' && (
                                <>
                                    ...
                                    {(lead.status === 'TROCA_TITULARIDADE' || lead.status === 'CADASTRANDO') && (
                                        <button
                                            onClick={() => setShowConverterModal(true)}
                                            disabled={actionLoading}
                                            className="w-full px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 flex items-center justify-center gap-2 disabled:opacity-50"
                                        >
                                            <UserCheck size={18} />
                                            Converter em Beneficiario
                                        </button>
                                    )}
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    type="number"
                                    value={converterForm.usina_id}
                                    onChange={(e) => setConverterForm({ ...converterForm, usina_id: e.target.value })}
                                    ...
                                    placeholder="ID da usina geradora"
                                />
                                <p className="text-xs text-slate-400 mt-1">
                                    Informe o ID da usina que fornecera os creditos
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    min={0}
                                    max={100}
                                    value={converterForm.desconto_percentual}
                                    onChange={(e) => setConverterForm({ ...converterForm, desconto_percentual: e.target.value })}
                                    className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
                                />
                                <p className="text-xs text-slate-400 mt-1">
                                    Desconto sobre a tarifa da concessionaria
                                </p>
                            </div>
                            ...
                            <div className="space-y-2">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={converterForm.criar_contrato}
                                        onChange={(e) => setConverterForm({ ...converterForm, criar_contrato: e.target.checked })}
                                ...
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={converterForm.enviar_convite}
                                        onChange={(e) => setConverterForm({ ...converterForm, enviar_convite: e.target.checked })}
                                        className="w-4 h-4 rounded border-slate-300 text-blue-500 focus:ring-blue-500"
                                    />
                                    <span className="text-sm text-slate-700 dark:text-slate-300">
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            ...
                            <button
                                ...
                                disabled={actionLoading || !converterForm.usina_id || !converterForm.uc_id}
